<?php
    ...
    class PDF extends FPDF{
        ...
        function Footer(){
                ...
                $this->Cell(0,5,utf8_decode("jopalmeros © Todos los derechos reservados."),0,0,"C");
        }   
    }
    ...
    $consulta = "SELECT * FROM confirmacion";
    ...
    */
    ...
    while ($row=$resultado->fetch_assoc()) {
        ...
        */
        $pdf->Text(55, 12, utf8_decode('ARQUIDIOCESIS DE XALAPA'));
        $pdf->Text(60, 20, utf8_decode('EL SEÑOR ARZOBISPO'));
        $pdf->Text(50, 28, utf8_decode('D. JORGE CARLOS PATRÓN WONG'));
        $pdf->Text(25,36, utf8_decode('ADMINISTRO EL SACRAMENTO DE CONFIRMACIÓN'));
        //$this->Text(78,33, utf8_decode('noexisteelemail@gamail.com'));

        $pdf->Image('img/ftv.jpg',162,5,33);
        ...
        */
        ...
        //***CONFIRMADO***
        //strtoupper = convierte cadena en mayúsculas.
        $nom_con = "A: ".utf8_encode(strtoupper($row['nom_con']));
        $pdf->Text(25, 45, utf8_decode($nom_con));
        //$pdf->Ln();
        $pdf->setlinewidth(0);    
        $pdf->line(25,46,195,46);
        ...
        */
        ...
        $padres = strtoupper("Hij".utf8_encode($row['hij'])." de ".utf8_encode($row['padre']));
        $pdf->SetFont('Arial','',12);
        $pdf->Text(25, 53, utf8_decode($padres));
        $pdf->line(25,55,195,55);
        //$pdf->Text(44, 60, utf8_decode("NIMBE ANNEL HERNANDEZ PEREZ"));

        $madre = utf8_encode(strtoupper($row['madre']));
        $pdf->Text(43, 60, utf8_decode($madre));
        $pdf->line(25,62,195,62);
        ...
        */
        ...
        $lugar_con = strtoupper("En la parroquia de: ". utf8_encode($row['lugar_confirmacion']));
        $pdf->SetFont('Arial','',12);
        $pdf->Text(25, 67, utf8_decode($lugar_con));
        $pdf->line(25,69,195,69);
        ...
        */
        ...
        $parroquia_bau = strtoupper("Bautizad". $row['hij']." en la parroquia de: ".utf8_encode($row['parroquia_bau']));
        $pdf->SetFont('Arial','',12);
        $pdf->Text(25, 74, utf8_decode($parroquia_bau));
        $pdf->line(25,76,195,76);
        ...
        */
        ...
        $lugar_bau = strtoupper("Lugar: ".utf8_encode($row['lugar_bau']));
        $pdf->SetFont('Arial','',12);
        $pdf->Text(25, 81, utf8_decode($lugar_bau));
        $pdf->line(25,83,195,83);
        ...
        */
        ...
        $lf_bau = strtoupper("Estado: ".utf8_encode($row['estado_bau'])." el día ".utf8_encode($row['dia_bau'])." de ".utf8_encode($row['mes_bau'])." de ". utf8_encode($row['ano_bau']));
        $pdf->SetFont('Arial','',12);
        $pdf->Text(25, 88, utf8_decode($lf_bau));
        $pdf->line(25,90,195,90);
        ...
        */
        ...
        $padrino = strtoupper("Padrino: ".utf8_encode($row['padrino']));
        $madrina = strtoupper("Madrina: ".utf8_encode($row['madrina']));
        $pdf->SetFont('Arial','',12);
        $pdf->Text(25, 95, utf8_decode($padrino));
        $pdf->line(25,97,195,97);
        $pdf->Text(25, 102, utf8_decode($madrina));
        $pdf->line(25,104,195,104);
        ...
        */
        ...
        $lf_con = strtoupper("Chiconquiaco, Ver el día "
                             .utf8_encode($row['dia_con'])
                             ." de "
                             .utf8_encode($row['mes_con'])
                             ." de "
                             .utf8_encode($row['ano_con']));
        $pdf->SetFont('Arial','',12);
        $pdf->Text(25, 109, utf8_decode($lf_con));
        $pdf->line(25,111,195,111);
